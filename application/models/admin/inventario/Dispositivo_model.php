...
class Dispositivo_model extends MY_Model {
    ...
    public $validate = array(
        ...
        array('field' => 'departamento_id', 'label' => 'Rol', 'rules' => 'trim|required'),
    );
    ...
    public function getAll() {
        return $this->db
                        ->select('U.*, UR.rol_id, R.descripcion, UD.departamento_id')
                        ->from($this->_table . ' U')
                        ->join('usuario_rol UR', 'UR.usuario_id = U.id')
                        ->join('Rol R', 'R.id = UR.rol_id')
                        ->join('usuario_departamento UD', 'UD.usuario_id = U.id')
                        ->get()
                        ->result();
    }
    ...
    public function getDato($id) {
        return $this->db
                        ...
                        ->join('usuario_departamento UD', 'UD.usuario_id = U.id AND U.id= ' . $id)
                        ->get()
                        ->row();
    }
    ...
    public function crear() {
        ...
        $this->db->insert('usuario_departamento', beforeInsert(array('usuario_id' => $usuario_id, 'departamento_id' => $this->input->post('departamento_id'))));
        return TRUE;
    }
